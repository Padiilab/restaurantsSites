...
export default class ReviewAdding extends Component {
            ...
            <div className={"contentHome"}>
                ...
                <div className={"addRestaurantWrapper"}>
                    ...
                    <StyledRating emptyIcon={<StarBorderIcon fontSize="inherit"/>} className={"ratingStar"}
                                  name="half-rating-read" defaultValue={5}
                                  onChange={(event) => this.setState({starRating: event.currentTarget.value})}
                                  precision={1}/>
                </div>
                <button onClick={this.addReview} type="button"
    ...
    addReview = () => {
        if (this.state.name) {
            axios.post(`https://restaurants-database.herokuapp.com/api/v1/restaurants/${this.props.editData.id}/reviews`, {
                name: this.state.name,
    ...
    renderAllReview = () => {
        if (!this.state.allReview) {
            axios.get(`https://restaurants-database.herokuapp.com/api/v1/restaurants/${this.props.editData.id}/reviews`)
                .then((response) => {
            ...
            this.state.allReview.forEach((element, index) => {
                reviewBox.push(
                    <div style={{height: 'fit-content'}} className="cardRest">
                        ...
                        <StyledRating emptyIcon={<StarBorderIcon fontSize="inherit"/>} className={"ratingStar"}
                                      name="half-rating-read" defaultValue={element.stars}
