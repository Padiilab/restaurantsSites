...
export const EditPage = ({ editData, onBackToHome }) => {
  ...
  const [webSite, setWebsite] = useState(editData.name);
  const [location, setLocation] = useState(editData.name);
  const [priceRating, setPriceRating] = useState(editData.name);
  const [id, setId] = useState(editData.name);
  ...
  const deleteCurrentRestaurant = () => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        axios
          .delete(`https://restaurants-database.herokuapp.com/api/v1/restaurants/${id}`)
          .then(response => {
            this.props.updateListRestaurant();
            this.props.onBackToHome();
